...
class ASRDataset(Dataset):
    def __init__(
        ...
        # TODO: Take subset
        subset_size      = int(len(self.fbank_files) * self.config['subset'])
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [feat.permute(1, 0) for feat, _, _ in batch]  # feat from (F, T) to (T, F)
        ...
        feat_lengths = torch.tensor([feat.shape[0] for feat in batch_feats])  # B
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.tensor([len(transcript) for transcript in batch_shifted])  # B  
