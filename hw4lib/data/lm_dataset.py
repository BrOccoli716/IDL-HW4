...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        self.text_files = [os.path.join(self.text_dir, path) for path in sorted(os.listdir(self.text_dir))]
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        lengths = torch.tensor([len(t) for t in shifted_transcripts])  # (B)  |  Here both transcripts should have the same length!
