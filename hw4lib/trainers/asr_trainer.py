...
class ASRTrainer(BaseTrainer):
    ...
    def _validate_epoch(self, dataloader):
        ...
        # TODO: Call recognize
        results = self.recognize(dataloader=dataloader)
        
        # TODO: Extract references and hypotheses from results
        references = [result['target'] for result in results]  # During validation, targets_golden is not None!
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                prompts = torch.full((batch_size, 1), self.tokenizer.sos_id, dtype=torch.long, device=feats.device)
