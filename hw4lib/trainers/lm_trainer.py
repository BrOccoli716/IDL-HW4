...
class LMTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            ...
            targets_shifted = targets_shifted.to(self.device)  # shape: (batch_size, max_len)
    ...
    def _validate_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.inference_mode():
                ...
                # Would you need to change the shape of the inputs to the criterion?
                # Hint: See the documentation for CrossEntropyLoss
                loss = self.criterion(raw_preds.reshape(-1, raw_preds.shape[-1]), targets_golden.reshape(-1))
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            if generation_config.get('top_k', 0) > 0 or generation_config.get('top_p', 0) > 0:
                ...
                seqs, scores = generator.generate_sample(prompts, temperature=generation_config['temperature'], top_k=generation_config['top_k'], top_p=generation_config['top_p'])
                ...
                seqs, scores = generator.generate_beam(prompts, beam_width=generation_config['beam_width'], temperature=generation_config['temperature'], repeat_penalty=generation_config['repeat_penalty'])
                ...
                seqs, scores = generator.generate_greedy(prompts, temperature=generation_config['temperature'], repeat_penalty=generation_config['repeat_penalty'])
