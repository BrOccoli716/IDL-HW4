...
class MultiHeadAttention:
    ...
    def __init__(self, embed_dim, num_heads):
        ...
        #  embed_dim -> embed_dim
        self.q_proj   = Linear(in_features=self.embed_dim, out_features=self.embed_dim)
        self.k_proj   = Linear(in_features=self.embed_dim, out_features=self.embed_dim)
        self.v_proj   = Linear(in_features=self.embed_dim, out_features=self.embed_dim)
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = np.broadcast_to(key_padding_mask[:, np.newaxis, np.newaxis, :], (self.N, self.num_heads, self.L, self.S))
        np.broadcast()
        
        # Expand attn_mask to (1, 1, L, S) and broadcast to (N, H, L, S)
        attention_mask = np.broadcast_to(attn_mask[np.newaxis, np.newaxis, :, :], (self.N, self.num_heads, self.L, self.S))
    ...
    def _split_heads(self, x):
        ...
        x = np.moveaxis(x, source=1, destination=2)
